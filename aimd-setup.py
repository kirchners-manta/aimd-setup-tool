...
parser.add_argument("-t" ,"--thermo", type=str, help="thermostat", default="nose")
...
if os.path.isdir(args.project):
    print("Project directory '" + args.project + "' already exists. Shall is be overwritten? [y/n]")
    answer = input()
...
if len(cp2k_infiles) != 4:
    sys.exit(" *** Error: There should be 4 CP2K input files in the project directory.")

# reorder the list of input files: geometry, equilibration, relaxation, production
cp2k_infiles = [cp2k_infiles[1], cp2k_infiles[0], cp2k_infiles[3], cp2k_infiles[2]]
...
for i, file in enumerate(cp2k_infiles):
    ...
    with open(file, "r") as f:
        ...
        if i == 0:
            lines = [line.replace("PROJECT_NAME project_name", "PROJECT_NAME " + str(args.project)) for line in lines]
            lines = [line.replace("BOX_LENGTH box_length", "BOX_LENGTH "+ str(args.boxsize)) for line in lines]
            lines = [line.replace("SIMBOX_XYZ simbox_xyz", "SIMBOX_XYZ " + str(args.coord)) for line in lines]
            with open(file, "w") as g:
                g.writelines(lines)
                
        # for the equilibration: adjust project name, box length, coord file, thermostat, temperature and number of steps
        elif i == 1:
            lines = [line.replace("PROJECT_NAME project_name", "PROJECT_NAME " + str(args.boxsize)) for line in lines]
            lines = [line.replace("BOX_LENGTH box_length", "BOX_LENGTH "+ str(args.boxsize)) for line in lines]
            lines = [line.replace("SIMBOX_XYZ simbox_xyz", "SIMBOX_XYZ " + str(args.coord)) for line in lines]
            lines = [line.replace("THERMO thermostat_type", "THERMO " + str(args.thermo)) for line in lines]
            lines = [line.replace("TEMP temperature", "TEMP " + str(args.t_equi)) for line in lines]
            lines = [line.replace("NSTEPS number_of_steps", "NSTEPS " + str(args.steps_equi)) for line in lines]
            with open(file, "w") as g:
                g.writelines(lines)
        ...
        # for the relaxation: adjust project name, box length, coord file, thermostat, temperature and number of steps
            ...
            lines = [line.replace("SIMBOX_XYZ simbox_xyz", "SIMBOX_XYZ " + str(args.coord)) for line in lines]
            lines = [line.replace("THERMO thermostat_type", "THERMO " + str(args.thermo)) for line in lines]
            lines = [line.replace("TEMP temperature", "TEMP " + str(args.t_relax)) for line in lines]
            lines = [line.replace("NSTEPS number_of_steps", "NSTEPS " + str(args.steps_relax)) for line in lines]
            with open(file, "w") as g:
                g.writelines(lines)
        ...
        # for the production: adjust project name, box length, coord file, thermostat, temperature and number of steps
            ...
            lines = [line.replace("SIMBOX_XYZ simbox_xyz", "SIMBOX_XYZ " + str(args.coord)) for line in lines]
            lines = [line.replace("THERMO thermostat_type", "THERMO " + str(args.thermo)) for line in lines]
            lines = [line.replace("TEMP temperature", "TEMP " + str(args.t_prod)) for line in lines]
            lines = [line.replace("NSTEPS number_of_steps", "NSTEPS " + str(args.steps_prod)) for line in lines]
            with open(file, "w") as g:
                g.writelines(lines)
